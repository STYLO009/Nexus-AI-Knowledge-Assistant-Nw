...
export default function ComparisonPage() {
  ...
  useEffect(() => {
    listKnowledgeDocuments().then(documents => setIndexedDocs(documents.filter(document => document.status === 'indexed'))).catch(() => setIndexedDocs([]));
  }, []);
  const [docA, setDocA] = useState(indexedDocs[0]?.id ?? '');
  const [docB, setDocB] = useState(indexedDocs[1]?.id ?? '');
  const [activeTab, setActiveTab] = useState<'overview' | 'differences' | 'recommendations'>('overview');
  ...
  return (
    <div className="flex-1 overflow-y-auto p-8">
      <div className="max-w-5xl mx-auto">
        <div className="mb-6">
          <h1 className="text-2xl font-bold" style={{ fontFamily: 'Outfit, sans-serif', color: '#f0f0ff' }}>Document Comparison</h1>
          <p className="text-sm" style={{ color: '#8888aa' }}>AI-powered semantic comparison with embedding-based analysis</p>
        </div>

        {/* Selector */}
        <div className="glass rounded-xl p-6 mb-6" style={{ border: '1px solid rgba(255,255,255,0.08)' }}>
          <div className="grid grid-cols-1 md:grid-cols-[1fr_auto_1fr] gap-4 items-center">
            <div>
              <label className="text-xs font-medium mb-1.5 block" style={{ color: '#8888aa', fontFamily: 'Inter, sans-serif' }}>DOCUMENT A</label>
              <select value={docA} onChange={e => setDocA(e.target.value)}
                className="w-full px-3 py-2.5 rounded-lg text-sm"
                style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', color: '#f0f0ff', fontFamily: 'Inter, sans-serif' }}>
                {indexedDocs.map(d => <option key={d.id} value={d.id} style={{ background: '#0a0a12' }}>{d.name}</option>)}
              </select>
            </div>
            <div className="text-center text-lg" style={{ color: '#8888aa' }}>⊞</div>
            <div>
              <label className="text-xs font-medium mb-1.5 block" style={{ color: '#8888aa', fontFamily: 'Inter, sans-serif' }}>DOCUMENT B</label>
          ...
          <div className="mt-4 flex items-center gap-3">
            <button onClick={compare} disabled={comparing || !docA || !docB || docA === docB}
              className="px-5 py-2.5 rounded-lg text-sm font-semibold transition-all duration-200"
              style={{
                background: comparing ? 'rgba(59,130,246,0.2)' : 'linear-gradient(135deg, #3b82f6, #8b5cf6)',
                color: comparing ? '#8888aa' : '#fff',
                fontFamily: 'Outfit, sans-serif',
                boxShadow: comparing ? 'none' : '0 0 16px rgba(59,130,246,0.3)',
              }}>
              {comparing ? (
                <span className="flex items-center gap-2">
                  <span className="w-3.5 h-3.5 border-2 border-white/30 border-t-white rounded-full animate-spin" />
                  Analyzing with embeddings…
                </span>
              ) : 'Run Comparison'}
            </button>
            {docA === docB && docA && (
              <span className="text-xs" style={{ color: '#f59e0b' }}>Select two different documents</span>
            )}
          </div>
        </div>
        ...
        {docAObj && docBObj && (
          <div className="grid grid-cols-2 gap-4 mb-6">
            {[docAObj, docBObj].map((doc, i) => (
              <div key={doc.id} className="glass rounded-xl p-4" style={{ border: `1px solid ${i === 0 ? 'rgba(59,130,246,0.2)' : 'rgba(139,92,246,0.2)'}` }}>
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-xs font-bold px-2 py-0.5 rounded" style={{ background: i === 0 ? 'rgba(59,130,246,0.2)' : 'rgba(139,92,246,0.2)', color: i === 0 ? '#60a5fa' : '#a78bfa', fontFamily: 'JetBrains Mono, monospace' }}>
                    {i === 0 ? 'A' : 'B'}
                  </span>
                  <span className="text-sm font-medium truncate" style={{ color: '#f0f0ff', fontFamily: 'Inter, sans-serif' }}>{doc.name}</span>
                </div>
                <div className="flex gap-3 text-xs" style={{ color: '#8888aa', fontFamily: 'JetBrains Mono, monospace' }}>
                  <span>{doc.chunks} chunks</span>
                  <span>{(doc.tokens / 1000).toFixed(1)}K tokens</span>
                </div>
                {doc.summary && <p className="mt-2 text-xs" style={{ color: '#8888aa', fontFamily: 'Inter, sans-serif' }}>{doc.summary.slice(0, 100)}…</p>}
              </div>
            ))}
          </div>
        )}
        ...
        {/* Result */}
        {result && (
          <div className="glass rounded-xl overflow-hidden fade-in" style={{ border: '1px solid rgba(255,255,255,0.08)' }}>
            {/* Tabs */}
            <div className="flex" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
              ...
              <div className="ml-auto flex items-center px-4 gap-2">
                ...
                <span className="text-xs" style={{ color: '#34d399', fontFamily: 'JetBrains Mono, monospace' }}>Embedding analysis complete</span>
              </div>
            </div>
            <div className="p-6 text-sm leading-relaxed" style={{ color: '#c4c4e0', fontFamily: 'Inter, sans-serif' }}>
            ...
            <div className="px-6 pb-6">
              <div className="rounded-lg p-4" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs font-medium" style={{ color: '#8888aa', fontFamily: 'Inter, sans-serif' }}>Semantic Similarity Score</span>
                  <span className="text-sm font-bold" style={{ color: '#60a5fa', fontFamily: 'JetBrains Mono, monospace' }}>
                    {(0.4 + Math.random() * 0.4).toFixed(3)}
                  </span>
                </div>
                <div className="h-1.5 rounded-full" style={{ background: 'rgba(255,255,255,0.06)' }}>
                  <div className="h-1.5 rounded-full" style={{ width: '62%', background: 'linear-gradient(90deg, #3b82f6, #8b5cf6)' }} />
                </div>
                <div className="flex justify-between text-xs mt-1" style={{ color: 'rgba(136,136,170,0.5)', fontFamily: 'JetBrains Mono, monospace' }}>
